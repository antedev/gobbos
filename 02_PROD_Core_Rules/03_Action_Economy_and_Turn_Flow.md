...
```
+-----------------------------------------------------------------------------+
|                         STANDARD ACTIONS CATALOG                            |
|   1. MOVE      2. ATTACK      3. PLUNDER      4. MANIPULATE      5. ORDER   |
+-----------------------------------------------------------------------------+
```
...
*   **Resolution**: Hits deal 1 damage/Wound per success threshold (or resolve via the **Overkill Rule** for Elites and the **Frontline Rule** for Mobs). Partial hits evaluate **Impact Size vs Target Size** for the **Staggered** condition (see [Combat Engine](05_Combat_Engine.md)).

### 3. Plunder (Boss or Mob)
Spend 1 action to pick up, scavenge, and secure loose **Loot**, **Scrap**, or physical objects in your current Zone:
*   Secured items are added to your personal carried **Bulk** or stashed in the Mob's inventory.
*   Looting heavy or bulky objects during combat consumes Bulk capacity and may trigger encumbrance penalties (see [The Raid Loop](09_The_Raid_Loop.md)).

### 4. Manipulate (Boss or Mob)
Spend 1 action to interact with physical mechanisms, search junk piles, bar doors, or trigger environmental hazards:
*   **Testing**: Roll the stat most relevant to the task (**Brains** for locks/traps, **Tough** for forcing doors/levers) against the **Zone Profile** or device TN.
*   **Player-Triggered Traps**: When you spend a Manipulate action to trigger an environmental hazard or trap against enemies (such as collapsing a ceiling or dropping a portcullis), make an activation test. On a success, roll the trap's listed **Damage Dice Pool**; each success on **5+** inflicts 1 damage or 1 Wound directly upon affected targets without GM rolls.

### 5. Order (Boss Only)
Spend 1 action (or 1 **Free Order**) to command a friendly **Mob** to spend its 2 actions for the round:
*   **Controlled Mobs**: Issuing orders to a Mob whose **Size** is $\le$ your current **Grunt** requires no dice roll.
*   **The 3 Mob States**:
    *   **Ordered**: The Mob executes up to 2 commanded actions. If ordered to spend only 1 action (e.g. Move), its remaining 1 action is saved for defense.
    *   **Loitering**: A controlled Mob that receives no orders spends 1 action rolling on the **Loitering Table** and saves 1 action for defense (granting 1d6 active Defence).
    *   **Out of Control**: A Mob that broke command spends both actions rolling on the **Out of Control Table** and saves 0 actions for defense.
*   **Regaining Control**: To restore order to an Out of Control Mob, you must spend a Standard Action to issue an Order, passing a command test of `Tough or Mouth 5+/[Mob Size]`.
...
### Reaction Types
1.  **Dodge (Boss Reaction)**: Spend 1 saved Standard Action. Roll your active **Slink** dice pool against the incoming attack's **Threat TN** in the **Clatter Roll**. Scoring successes $\ge$ Threat TN completely negates the attack (0 damage).
2.  **Parry (Boss Reaction)**: Spend 1 saved Standard Action. Roll your active **Tough** dice pool against the incoming attack's **Threat TN** in the **Clatter Roll**. Requires an equipped **Shield** or **Heavy Weapon**. Scoring successes $\ge$ Threat TN completely negates the attack (0 damage).
3.  **Scatter (Mob Reaction)**: Spend 1 saved Boss Standard Action (or 1 unused Free Order) to order a targeted Mob with $\ge 1$ unused action to disperse. Roll your Boss's **Mouth** pool against `Threat TN + (Mob Size - 1)`. Success negates damage and scurries the Mob 1 Zone into cover.
4.  **Reactive Quirks**: Specific Boss abilities (such as *Meat Shield* or *Ankle Bite*) trigger out of turn by paying their stated Grunt or Reaction cost.
...
[Phase 0: Setup] (First Round Only)
       ...
       v
+--> [Phase 1: Round Start]
|      - Evaluate Start-of-Round conditions & triggers
|      - Spawn reinforcements & update active Raid Points
|      |
|      v
|    [Phase 2: Player Active Turn]
|      - Bosses declare Standard Actions, Move, Attack, Plunder, Manipulate
|      - Bosses issue Orders (using Free Orders & Standard Actions)
|      - Resolve Unordered Mobs (Loitering & Out of Control tables)
|      - GM declares enemy reactions (if applicable)
|      |
|      v
|    [Phase 3: Enemy Active Turn]
|      - Deterministic enemy threat resolution (GM never rolls)
|      - Players execute Clatter Rolls (Dodge/Parry) using saved actions
|      - Bosses issue reactive "Scatter!" orders using saved actions
|      |
|      v
|    [Phase 4: Round Closure]
|      - Tally Raid Points & process End-of-Round condition/hazard ticks
|      - Automatically remove all Staggered conditions from all units
|      - Resolve Swarm Terror Morale checks (if 50% casualties occurred)
|      - Reset action budgets for all Bosses and Mobs
|      |
+------+ (If combat continues; otherwise proceed to Phase 5)
       |
       v
...
*   **Disengaging from Melee**: Leaving a Zone containing alert enemies without fighting triggers an immediate Opportunity Attack. To disengage safely, a Boss or Mob must spend 1 Standard Action to **Disengage**, testing:
    $$\text{Slink} \quad \text{5+/[Highest Enemy Defence TN]}$$
